...
func (w *identifierWalker) Visit(node ast.Node) ast.Visitor {
	...
	case *ast.Ident:
		...
		// FIXME instead of manually creating a new node, clone it and trim the node from its comments and position https://github.com/andrealim11/go-mutesting/issues/49
	...
	case *ast.SelectorExpr:
		...
		if initialize {
			// FIXME we need to clone the node and trim comments and position recursively https://github.com/andrealim11/go-mutesting/issues/49
